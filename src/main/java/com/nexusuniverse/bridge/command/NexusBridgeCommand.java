...
public final class NexusBridgeCommand implements CommandExecutor, TabCompleter {
    ...
    private void showStatus(CommandSender sender) {
        ...
        sender.sendMessage("§7HTTP sent / failed / dropped / queued: §f"
            + plugin.http().sentCount() + " / "
            + plugin.http().failedCount() + " / "
            + plugin.http().droppedCount() + " / "
            + plugin.http().queuedCount());
        sender.sendMessage("§7Circuit breaker: " + (plugin.http().circuitOpen()
            ? "§cOPEN §7(" + plugin.http().circuitRemainingMs() + "ms remaining)"
            : "§aCLOSED"));
    ...
    private void ping(CommandSender sender) {
        ...
        sender.sendMessage(PREFIX + "§ePinging Kairos health endpoint...");

        plugin.http().get("/", response -> {
            long elapsedMs = Math.max(0L, (System.nanoTime() - started) / 1_000_000L);
            if (response.ok()) {
                sender.sendMessage(PREFIX + "§aKairos is reachable in §f" + elapsedMs + " ms §7(HTTP " + response.status() + ").");
            } else {
                sender.sendMessage(PREFIX + "§cKairos is not reachable. §7" + failureText(response));
            }
        });
    }
    ...
    private void showQueue(CommandSender sender) {
        sender.sendMessage(PREFIX + "§7HTTP queue: §f" + plugin.http().queuedCount()
            ...
            + " §7| dropped: §f" + plugin.http().droppedCount());
    }
